...
（此处可插入游戏运行截图——标题画面、战斗场景、HUD 显示、胜利/失败结算各一张）


## 4.11 总体心得与个人感悟

参与 AlleyFist 项目的 View 层开发，在以下方面收获较大：
...
**智能体的使用。** 本项目在开发过程中使用了 Claude Code 智能体辅助编码，主要用于三个方面：根据 Common 接口批量生成 View 层的绘制代码骨架和 HUD 布局；在接口变更时全局搜索并替换过时的类型引用；协助编写本报告的初稿框架。智能体的优势在于能快速处理重复性的代码生成和全局重构任务，但在理解游戏视觉风格细节和对齐多成员并行修改的版本冲突方面，仍需要人工判断和团队沟通。智能体的使用也存在一定局限：生成的代码结构有时过于平铺，缺少合理的函数拆分；对 Qt API 的版本差异不够敏感。


## 4.12 个人反思与收获

作为 View 层开发者，本次项目的最大收获是：好的分层设计不是"写完代码后再划分"，而是"写之前就想清楚每一层的输入和输出是什么"。View 层的输入是 `const GameState*` 和十一个 `std::function`，输出是像素和命令回调。界定清楚这些之后，不管 Common 怎么变、ViewModel 怎么重构，View 层的核心逻辑不需要大改。
